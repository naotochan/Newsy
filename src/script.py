...
- {assistant}（アシスタント / 好奇心旺盛な聞き手）: リスナーの代弁者。驚き・疑問・発見を素直に表現する。「え、ちょっと待って」「へえ、知らなかった！」「まじですか」「いや〜それは気になる」と割り込み、深掘りの質問をする。
...
- 「私たちの生活にどう関係するんだろう」「つまり何が変わるの？」という視点を必ず入れる
- **「それって」を多用しない**。代わりに「ということは」「つまり」「なるほど、じゃあ」「要するに」「ってことは」など多彩なつなぎ表現を使い分ける

## 各トピックの構成（この順番で展開する）
1. **記事紹介**: ホストが記事の内容・背景をわかりやすく紹介する（1〜2往復）
2. **議論・深掘り**: 二人で「なぜ重要か」「技術的にどうすごいか」「業界への影響」を掘り下げる（3〜5往復）
3. **リスナーへの問いかけ**: 最後に「皆さんはどう思いますか？」「こういう使い方してる人いますかね？」など、聞き手に考えてもらう一言で締める（1往復）

## 構成ルール
- 1つの記事につき8〜12往復の掛け合いでじっくり深掘りする
- トピック間は有機的に繋ぐ（「そういえばさ」「それで思い出したんだけど」「ちょうどそれに関連して」）
- {opening_rule}
- パート末尾に次のトピックや次パートの予告（「次は〜について」「続いては〜を」など）を入れない
- 番組全体で15分程度（読み上げ時）になる量にする
- 各発言は1〜3文に収める
...
def _call_llm(prompt: str, config_path: str = "config/settings.yaml") -> str:
    """設定に基づいて LLM を呼び出す共通関数"""
...
def select_articles(
    ...
    """LLM で重要度を判定し、上位 max_select 本の記事を選定する"""
    if len(articles) <= max_select:
        return articles

    # 記事一覧を番号付きで作成
    listing = []
    for i, a in enumerate(articles, 1):
        summary = (a.content or a.summary or "")[:100]
        listing.append(f"{i}. [{a.source}] {a.title}\n   {summary}")
    listing_text = "\n".join(listing)

    prompt = f"""\
以下は今日取得したニュース記事の一覧です。
この中から、テクノロジー・AIに関心のあるリスナーにとって最も重要かつ興味深い記事を{max_select}本選んでください。
...
記事の番号のみをカンマ区切りで返してください（例: 3,7,12）。
番号以外の文字は一切含めないでください。
...
"""
    ...
    try:
        response = _call_llm(prompt, config_path)
        # レスポンスから番号をパース
        import re
        numbers = [int(n) for n in re.findall(r"\d+", response)]
        selected = []
        for n in numbers:
            if 1 <= n <= len(articles) and articles[n - 1] not in selected:
                selected.append(articles[n - 1])
            if len(selected) >= max_select:
                break
        if selected:
            return selected
    except Exception as e:
        print(f"  [警告] 記事選定でエラー: {e}")

    # フォールバック: 先頭 max_select 件
    print("  [フォールバック] 先頭の記事を使用します。")
    return articles[:max_select]
